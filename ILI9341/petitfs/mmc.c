...
void SDCARD_activate(void)
{
	//SPCR |= (1 << SPR1)|(1 << SPR0);										// F_CPU/64 = 300kHz @20MHz F_CPU
	#if USE_SDCARD_CS == 1													// If SDCARD_CS in use
	SD_CS_LO;
	#endif
}

void SDCARD_deactivate(void)
{
	#if USE_SDCARD_CS == 1													// If SDCARD_CS in use
	SD_CS_HI;
	#endif
	SPCR &= ~((1 << SPR1)|(1 << SPR0));										// F_CPU/2
}
